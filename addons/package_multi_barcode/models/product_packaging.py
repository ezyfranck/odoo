...
class ProductPackage(models.Model):
    ...
    barcode_ids = fields.One2many(
        comodel_name="product.packaging.barcode",
    ...
    def _compute_barcode(self):
        for package in self:
            package.barcode = package.barcode_ids[:1].name

    def _inverse_barcode(self):
        for package in self:
            if package.barcode_ids:
                package.barcode_ids[:1].write({"name": package.barcode})
            elif not package.barcode:
                package.barcode_ids.unlink()
            else:
                self.env["product.packaging.barcode"].create(
    ...
    def _search(self, domain, *args, **kwargs):
        ...
        return super(ProductPackage, self)._search(domain, *args, **kwargs)

    def _get_barcode_domain(self, sub_domain, domain):
        barcode_operator = sub_domain[1]
        barcode_value = sub_domain[2]
        barcodes = self.env["product.packaging.barcode"].search(
